...
import simplepdl.RessourceUsage;
...
public class SimplePDLValidator extends SimplepdlSwitch<Boolean> {
	...
	public Boolean caseWorkSequence(WorkSequence object) {
		...
		this.result.recordIfFailed(
				object.getProcess().getProcessElements().stream()
					...
					.allMatch(pe -> (pe.equals(object) || !(((WorkSequence) pe).getPredecessor().equals(object.getPredecessor()) & ((WorkSequence) pe).getSuccessor().equals(object.getSuccessor()) & ((WorkSequence) pe).getLinkType().equals(object.getLinkType())))),
				object, 
				"Le nom de l'activité (" + object.getPredecessor().getName() + "--" + object.getLinkType().toString()+"->"+object.getSuccessor().getName()+ ") n'est pas unique");
	...
	public Boolean caseGuidance(Guidance object) {
		...
		this.result.recordIfFailed(
				object.getText() != null, 
				object, 
				"Le nom du guidance ne doit pas être vide");
		return null;
	}
	@Override
	public Boolean caseRessourceUsage(RessourceUsage object) {
		// Contraintes sur RessourceUsage
		this.result.recordIfFailed(
				object.getAmount()<= object.getRessource().getQuantity(), 
				object, 
				"Le nombre des ressources est inférieur aux nombre des ressources utilisés par le workdefinition");
		this.result.recordIfFailed(
				!(object.getAmount()<=0),
				object, 
				"La quantité d'un ressourceUsage ne doit pas être négative");
		return null;
	}
	@Override
	public Boolean caseRessource(Ressource object) {
		...
		this.result.recordIfFailed(
				object.getName() != null || object.getName().matches(IDENT_REGEX), 
				object, 
				"Le nom de ressource ne doit pas etre vide");
		this.result.recordIfFailed(
				!(object.getQuantity()<=0),
				object, 
				"La quantité d'une ressource ne doit pas être négative");
		this.result.recordIfFailed(
				object.getProcess().getProcessElements().stream()
