...
type info =
  ...
  | InfoEnumConst of string * int * string
...
let string_of_info info =
  ...
  | InfoFun (n,t,tp,_) -> "Fonction "^n^" : "^(List.fold_right (fun elt tq -> if tq = "" then (string_of_type elt) else (string_of_type elt)^" * "^tq) tp "" )^
                      " -> "^(string_of_type t)
  | InfoEnumConst (n, value, enumName) -> "EnumConst "^n^" of Enum "^enumName^" : "^(string_of_int value)
...
let modifier_type_variable t i =
    match !i with
    | InfoVar (n,_,dep,base,_isRef) -> i:= InfoVar (n,t,dep,base, _isRef)
...
let modifier_type_fonction t tp lr i =
       match !i with
       | InfoFun(n,_,_,_) -> i:= InfoFun(n,t,tp,lr)
       | _ -> failwith "Appel modifier_type_fonction pas sur un InfoFun"

let%test _ = 
  let info = InfoFun ("f", Undefined, [], []) in
  let ia = info_to_info_ast info in
  modifier_type_fonction Rat [Int ; Int] [false; false] ia;
  match info_ast_to_info ia with
  | InfoFun ("f", Rat, [Int ; Int], [false; false]) -> true
  | _ -> false
 
(* Modifie l'emplacement (dépl, registre) si c'est une InfoVar, ne fait rien sinon *)
 let modifier_adresse_variable d b i =
     match !i with
     |InfoVar (n,t,_,_,_isRef) -> i:= InfoVar (n,t,d,b,_isRef)
