...
let%test_unit "testTypePointeurOk" = 
  let _ = compiler (pathFichiersRat^"testTypePointeurOk.rat") in ()

let%test_unit "testTypeNullOk" = 
  let _ = compiler (pathFichiersRat^"testTypeNullOk.rat") in ()

let%test_unit "testTypeNewOk" = 
  let _ = compiler (pathFichiersRat^"testTypeNewOk.rat") in ()

let%test_unit "testTypeIncompatibleAffectation" = 
  try 
    let _ = compiler (pathFichiersRat^"testTypeIncompatibleAffectation.rat")
    in raise ErreurNonDetectee
  with
  | TypeInattendu(_, _) -> ()

let%test_unit "testTypeDerefNonPointeur" = 
  try 
    let _ = compiler (pathFichiersRat^"testTypeDerefNonPointeur.rat")
    in raise ErreurNonDetectee
  with
  | TypeInattendu(_, _) -> ()

let%test_unit "testTypeAffectationDeref" = 
  let _ = compiler (pathFichiersRat^"testTypeAffectationDeref.rat") in ()

let%test_unit "testTypeAffectationDerefErreur" = 
  try 
    let _ = compiler (pathFichiersRat^"testTypeAffectationDerefErreur.rat")
    in raise ErreurNonDetectee
  with
  | TypeInattendu(Bool, Int) -> ()

let%test_unit "testTypeDoublePointeur" = 
  let _ = compiler (pathFichiersRat^"testTypeDoublePointeur.rat") in ()

(* Tests procédures *)

let%test_unit "testTypeProcedureSimple" =
  let _ = compiler (pathFichiersRat^"testTypeProcedureSimple.rat") in ()

let%test_unit "testTypeProcedureRetourVoid" =
  let _ = compiler (pathFichiersRat^"testTypeProcedureRetourVoid.rat") in ()
...
let%test_unit "testTypeProcedureParametres" =
  let _ = compiler (pathFichiersRat^"testTypeProcedureParametres.rat") in ()

let%test_unit "testTypeProcedureParametresIncorrects" =
  try
    let _ = compiler (pathFichiersRat^"testTypeProcedureParametresIncorrects.rat")
    in raise ErreurNonDetectee
  with
...
let%test_unit "testTypeProcedureMultipleRetours" =
  let _ = compiler (pathFichiersRat^"testTypeProcedureMultipleRetours.rat") in ()

(* Les autres Tests sont dans avec_fonction *)
