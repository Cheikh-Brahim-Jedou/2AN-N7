...
(*tests des pointeurs *)
let%test_unit "testPointeurDeclaration" = 
  let _ = compiler (pathFichiersRat^"testPointeurDeclaration.rat") in ()

let%test_unit "testPointeurUtilisationAvantDeclaration" = 
  try 
    let _ = compiler (pathFichiersRat^"testPointeurUtilisationAvantDeclaration.rat")
    in raise ErreurNonDetectee
  with
  | IdentifiantNonDeclare("p") -> ()

let%test_unit "testPointeurDoubleDeclaration" = 
  try 
    let _ = compiler (pathFichiersRat^"testPointeurDoubleDeclaration.rat")
    in raise ErreurNonDetectee
  with
  | DoubleDeclaration("p") -> ()

let%test_unit "testPointeurAdresseNonDeclare" = 
  try 
    let _ = compiler (pathFichiersRat^"testPointeurAdresseNonDeclare.rat")
    in raise ErreurNonDetectee
  with
  | IdentifiantNonDeclare("x") -> ()
...
let%test_unit "testProcedureParametres" =
  let _ = compiler (pathFichiersRat^"testProcedureParametres.rat") in ()

let%test_unit "testProcedureDoubleDeclaration" =
  try
    let _ = compiler (pathFichiersRat^"testProcedureDoubleDeclaration.rat")
    in raise ErreurNonDetectee
  with
  | DoubleDeclaration("afficher") -> ()

let%test_unit "testProcedureNonDeclare" =
  try
    let _ = compiler (pathFichiersRat^"testProcedureNonDeclare.rat")
    in raise ErreurNonDetectee
  with
  | IdentifiantNonDeclare("afficher") -> ()

let%test_unit "testRetourVoidDansMain" =
  try
    let _ = compiler (pathFichiersRat^"testRetourVoidDansMain.rat")
    in raise ErreurNonDetectee
  with
  | RetourDansMain -> ()

let%test_unit "testProcedureMultiple" =
  let _ = compiler (pathFichiersRat^"testProcedureMultiple.rat") in ()

let%test_unit "testProcedureAppelDansProcedure" =
  let _ = compiler (pathFichiersRat^"testProcedureAppelDansProcedure.rat") in ()
